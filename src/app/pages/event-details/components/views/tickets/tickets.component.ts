        ...
        @for (ticket of tickets(); track $index) {
          <div class="ticket flex flex-col lg:flex-row gap-3 justify-between items-start lg:items-center px-7 py-5 rounded-lg bg-[#FFECF0]">
            <div class="info flex gap-7 items-center">
              <p class="type text-base !font-['Montserrat']">{{ ticket.type }}</p>
              <p class="price font-bold !font-['Inter] text-2xl">{{ ticket.price | aoMoneyFormat }} KZ</p>
            </div>
...
export class TicketsComponent implements OnInit {
  ...
  theEvent = {
    name: 'Mulheres e as tecnotecnologias',
  ...
  ticketsAvailableToCheckout = computed(() => {
    return this.tickets().flatMap(ticket => ({ event: this.theEvent, ticket: ticket, quantity: 1 }))
  });
